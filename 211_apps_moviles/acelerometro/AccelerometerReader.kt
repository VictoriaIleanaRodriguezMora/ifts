...
// todo se hace atraves de sensor manager
    ...
    // recibe un ObjetoAccelerometerMeasurement y devuelve Unit
    /** Inicia la suscripción al sensor y comienza a recibir mediciones.
        @param onMedicionChanged Función lambda que se ejecutará al recibir nuevos datos.
    ...
    fun iniciarLecturaDelSensor(onMedicionChangedParam: (ObjetoAccelerometerMeasurement) -> Unit): Boolean {
        ...
        if (acelerometroDelDispositivo == null) { return false }
        val sensor = acelerometroDelDispositivo
    ...
    // Cada vez que se mueve el celular Android ejecuta automáticamente:
    override fun onSensorChanged(event: SensorEvent?) { // Android lo llama solo
        ...
        if (event.sensor.type == Sensor.TYPE_ACCELEROMETER) {
            ...
            // Empaquetamos los datos en nuestro modelo de datos personalizado. Que recibe x, y, z
            val ObjetoMedidas = ObjetoAccelerometerMeasurement(x = xEv, y = yEv, z = zEv)

            // Usa la llamada segura (?.) para invocar el callback solo si no es nulo. ejecuta esto solamente si no es null
            // invoke pasará las mediciones hacia la clase que haya llamado a 'startReading'
            onMedicionChanged?.invoke(ObjetoMedidas) // onMedicionChanged(ObjetoMedidas)
           /* if (onMedicionChanged != null) {
                onMedicionChanged.invoke(ObjetoMedidas)
            } */
            // es el puente entre la capa que lee el sensor y la capa que muestra la información en pantalla. Sin esta línea, el acelerómetro seguiría leyendo valores, pero nunca llegarían a la UI
            // ---------------------------------------------------------------------------------
